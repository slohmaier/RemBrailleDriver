...
from .remBrailleCom import RemBrailleCom, REMBRAILLE_PORT
from .hostDetection import get_vm_host_ip, suggest_host_ips
...
class RemBrailleDriverSetting:
	...
	class AutoConnectSetting(BooleanDriverSetting):
		"""Auto-connect on startup setting"""
		id = "autoConnect"
		displayName = _("Automatically connect on startup")
		defaultVal = True
...
class BrailleDisplayDriver(braille.BrailleDisplayDriver):
	...
	# Driver settings
	supportedSettings = (
		RemBrailleDriverSetting.AutoConnectSetting(),
	)
	...
	def _load_settings(self):
		...
		try:
			section = config.conf["braille"]["remBraille"]
			self._hostIP = section.get("hostIP", "")
			self._port = section.get("port", REMBRAILLE_PORT)
			self._autoConnect = section.get("autoConnect", True)
		except KeyError:
			# Create default config section
			if "remBraille" not in config.conf["braille"]:
				config.conf["braille"]["remBraille"] = {}
	...
	def _save_settings(self):
		...
		try:
			...
			section["autoConnect"] = self._autoConnect
			config.conf.save()
	...
	def _connect_to_host(self, host_ip: str, port: int = REMBRAILLE_PORT) -> bool:
		...
		with self._connection_lock:
			...
			if self.com.connect(host_ip, port):
				...
				wx.CallAfter(
					gui.messageBox,
					_("Successfully connected to RemBraille host at {ip}:{port} with {cells} cells.").format(
						ip=host_ip, port=port, cells=self.numCells
					),
					_("RemBraille Connected"),
	...
	def _show_connection_dialog(self, auto_connect: bool = False):
		...
		try:
			...
			if auto_connect:
				wx.CallAfter(
					gui.messageBox,
					_("Could not auto-detect RemBraille host. Please configure the connection manually in NVDA Settings > Braille."),
					_("RemBraille Auto-Connect Failed"),
...
class RemBrailleInputGesture(inputCore.InputGesture):
	...
	def __init__(self, gesture_name: str):
		super().__init__()
		self.id = f"remBraille:{gesture_name}"
...
class RemBrailleConnectionDialog(wx.Dialog):
	...
	def _setup_ui(self):
		...
		self.auto_connect_cb.SetValue(self.driver._autoConnect)
	...
	def _on_connect(self, event):
		...
		self.driver._port = port
		self.driver._autoConnect = auto_connect
